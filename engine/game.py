...
class GameState:
    ...
    def apply_play_result(self, result):
        ...
        if self.down == 4 and not self.going_for_it:
            ...
            if in_fg_range and not go_for_it:
                ...
                self.going_for_it = True
                return "going_for_it"

        if self.down == 4 and self.going_for_it:
            self.going_for_it = False
            if self.yards_to_go <= 0:
                # Converted — 1st & 10, keep possession
                self.down = 1
                self.yards_to_go = config.FIRST_DOWN_YARDS
                return "first_down"
            else:
                # Failed to convert — turnover on downs
                self.flip_possession()
